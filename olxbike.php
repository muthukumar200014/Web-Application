         ...
         <div class="w3-main" style="margin-left:10px">
				...
				<?php 
						...
						if ($min!=0 && $max!=0 && $min<=$max) {
								$res=mysqli_query($link,"SELECT * from images WHERE category='Bikes' and price BETWEEN '$min' and '$max' ORDER BY id DESC");
				...
				<?php
						...
						<?php } }
						elseif($min>$max){
							echo "<script type='text/javascript'>alert('Enter the correct values.');window.location.href='olxbike.php';</script>";
						}	}
						
							else {
							$res=mysqli_query($link,"SELECT * from images WHERE category='Bikes' ORDER BY id DESC");
								foreach ($res as $image1) {
								$temp = $image1["image"];
								$b = str_replace(',', ' ', $temp );
								$str_arr = explode(" ", $b); 
